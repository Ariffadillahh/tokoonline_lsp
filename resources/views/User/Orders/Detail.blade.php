            ...
            @else
                <article class="my-5">
                    <div class="flex items-center mb-4">
                        <img class="w-10 h-10 me-4 rounded-full" src="{{ asset('storage/image_user/' . Auth::user()->image) }}"
                            alt="">
                        <div class="font-medium dark:text-white">
                            <p>{{$item->name_penerima}} <p 
                                    class="block text-sm text-gray-500 dark:text-gray-400"> {{ \Carbon\Carbon::parse($item->waktu_rate)->diffForHumans() }}</p>
                            </p>
                        </div>
                    </div>
